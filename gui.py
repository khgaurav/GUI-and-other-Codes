...
def digitalcam10():
	...
	proc = subprocess.Popen(["xdotool search --onlyvisible --name frame"], stdout=subprocess.PIPE, shell=True)
	(out, err) = proc.communicate()
	#print out
	cmd="xdotool windowmove "+out.strip()+" 1366 768 &"
	os.system(cmd)
...
def motorcode():
	...
	sleep(1)
	proc = subprocess.Popen(["xdotool search --onlyvisible --name Python\ 2.7.15"], stdout=subprocess.PIPE, shell=True)
	(out, err) = proc.communicate()
	#print out
	cmd="xdotool windowsize "+out.strip()+" 532 100 && xdotool windowmove "+out.strip()+" 1366 0 &"
	os.system(cmd)
	os.system("xdotool search --name \"Python 2.7.15+ Shell\" set_window --name \"Motorcode\"")
